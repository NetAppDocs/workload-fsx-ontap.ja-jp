...
* S3アクセスポイントのネットワーク設定を*インターネット*に設定します。link:edit-s3-access-point.adoc["S3アクセスポイントのネットワーク構成を編集"].
...
いずれかの手順が失敗した場合は、<<Troubleshoot the infrastructure setup,ジャーナルテーブルのインフラストラクチャ設定のトラブルシューティング>>またはNetAppサポートにお問い合わせください。
