...
---
= NetApp Workload Factory でファイルシステムの容量を手動で調整する
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
使用量が指定したしきい値を超えた場合は、SSD ストレージ容量を増やします。ワーキング セットを使用していないときは SSD ストレージ容量を減らして、コストを節約します。

あるいは、link:manage-capacity-automatically.html["自動容量管理を有効にする"]して、Workload Factory がファイル システムの容量を管理できるようにします。
