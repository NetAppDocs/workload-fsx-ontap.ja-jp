...
---
= Amazon FSx for NetApp ONTAP in NetApp Workload Factoryの既知の制限事項
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
既知の制限事項は、本製品のこのリリースでサポートされていないプラットフォーム、デバイス、または機能、もしくは正しく相互運用できないものを示します。以下の制限事項は、NetApp Workload Factory における Amazon FSx for NetApp ONTAP に特有のものです。これらの制限事項を十分にご確認ください。
...
* iSCSI ボリュームではボリューム サイズの縮小はサポートされていません。
...
== Amazon S3 アクセスポイントの制限

ストレージ VM あたりの S3 アクセス ポイントの数の制限は 4,000 です。



[[limitations-for-storage-vm-migration-replication]]
== ストレージVM移行レプリケーションの制限事項

* Workload Factoryは、第1世代と第2世代のFSx for ONTAPファイルシステム間でのストレージVMの移行レプリケーションをサポートしていません。
* S3アクセスポイントまたはONTAP S3設定（vserver上のobject-store-server、バケット、ユーザー、グループ、ポリシーなど）を含むボリュームでは、カットオーバーレプリケーションはサポートされていません。
