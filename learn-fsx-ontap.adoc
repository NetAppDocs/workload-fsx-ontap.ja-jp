...
お客様のFSx for ONTAPアカウントは、ワークロードファクトリではなくAWSによって管理されます。を参照してください link:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/what-is-fsx-ontap.html#pricing-for-fsx-ontap["Amazon FSx for NetApp ONTAPの価格設定"^]。
