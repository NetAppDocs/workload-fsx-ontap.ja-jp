...
summary: オンプレミスの ONTAP データを NetApp Workload Factory で検出およびレプリケートし、クラウド上のバックアップとして使用できるようにします。 
---
...
[role="lead"]
オンプレミスの ONTAP データを NetApp Workload Factory で検出およびレプリケートし、クラウド上のバックアップとして使用できるようにします。
...
* 検出されたオンプレミスクラスタに関連付けるための接続済みリンク。リンクがない場合は、link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["作成"^]必要があります。
...
.. *ONTAP認証情報*：以下を入力してください：
+
*** GovCloud アカウント：ONTAP 資格情報を含む ARN を入力してください。
*** その他のアカウント：オンプレミス ONTAP クラスタの ONTAP 認証情報を入力してください。ユーザーが必要な権限を持っていることを確認してください。
...
* オンプレミス ONTAP クラスターと、レプリケーション関係に使用する FSx for ONTAP ファイルシステムの両方に、関連付けられたリンクが必要です。link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["既存のリンクを関連付ける方法、または新しいリンクを作成して関連付ける方法を学びます"^]。リンクが関連付けられたら、この操作に戻ってください。
